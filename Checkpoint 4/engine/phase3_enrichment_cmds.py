...
class EnrichmentContext:
    ...
    def repo_root(self) -> Path:
        # data_dir is Checkpoint 4/data → parents: CP4, repo root
        return self.data_dir.resolve().parent.parent


def _repo_relpath(ctx: EnrichmentContext, path: Path) -> str:
    """Prefer repo-relative paths in manifests (portable; no /Users/... leak)."""
    try:
        return str(path.resolve().relative_to(ctx.repo_root))
    except ValueError:
        return str(path)
...
def cmd_acquire(
    ...
    try:
        ...
        dens_disp = _repo_relpath(ctx, dens_path)
        enriched_disp = _repo_relpath(ctx, enriched_path)
        active_disp = _repo_relpath(ctx, Path(active))
        manifest = {
            "label": label,
            "adapter": adapter,
            "entity_meta": entity_meta,
            "density_path": dens_disp,
            "enriched_frame": enriched_disp,
            "active_frame": active_disp,
        ...
        append_bus_message(
            ...
            note=f"active_frame={active_disp}",
        )
        ...
        ctx.append_decision(
            out_dir, {"event": "enrich", "label": label, "frame": active_disp}
        )
        print(f"Acquisition OK → active frame {active_disp}")
